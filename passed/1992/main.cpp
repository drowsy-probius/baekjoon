...
void check(int *arr, int x, int y, int delta, int n){
    int tmp = *(arr + x*n + y);
    bool flag = true;
        ...
        for(int i=x; i<x+delta; i++){
            ...
            for(int j=y; j<y+delta; j++){
                if( tmp != *(arr + i*n + j) ){
                    ...
                    cout << '(';                    
                    check(arr, x,           y,          delta/2, n);
                    check(arr, x,           y+delta/2,   delta/2, n);
                    check(arr, x+delta/2,    y,          delta/2, n);
                    check(arr, x+delta/2,    y+delta/2,   delta/2, n);
                    cout << ')';
