...
void DFS(vector<pair<int, int>> graph[], int root, bool isVisited[], int totalWeight)
{
  isVisited[root] = true;
  ...
  for(auto it=graph[root].begin(); it!=graph[root].end(); it++){
    if(!isVisited[it->first]){
      DFS(graph, it->first, isVisited, totalWeight + it->second);
    }
  }
}
...
int main(void)
{
  ...
  int startHead, startTail, startWeight = 0;

  for (int i = 0; i < n; i++)
  {
    int root, child, distance;
    cin >> root >> child;
    while (child != -1)
    {
      cin >> distance;
      graph[root].push_back(make_pair(child, distance));
      cin >> child;
    }
  }

  DFS(graph, 1, isVisited, 0);

  diameter = 0;
  fill(isVisited, isVisited + n + 1, false);

  DFS(graph, farthestNode, isVisited, 0);
